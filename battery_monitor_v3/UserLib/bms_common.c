    ...
    switch (bms->status.state) {
        case BMS_STATE_OFF:
            ...
            if (bms_dis_allowed(bms)) {
                ...
                LOGI("OFF -> DIS (error flags: 0x%08x)", bms->status.error_flags);
            }
            else if (bms_chg_allowed(bms)) {
                bms_chg_switch(bms, true);
                bms->status.state = BMS_STATE_CHG;
                LOGI("OFF -> CHG (error flags: 0x%08x)", bms->status.error_flags);
            }
            break;
        case BMS_STATE_CHG:
            if (!bms_chg_allowed(bms)) {
                ...
                LOGI("CHG -> OFF (error flags: 0x%08x)", bms->status.error_flags);
            }
            else if (bms_dis_allowed(bms)) {
                bms_dis_switch(bms, true);
                bms->status.state = BMS_STATE_NORMAL;
                LOGI("CHG -> NORMAL (error flags: 0x%08x)", bms->status.error_flags);
            }
        ...
        case BMS_STATE_DIS:
            if (!bms_dis_allowed(bms)) {
                ...
                bms->status.state = BMS_STATE_OFF;
                LOGI("DIS -> OFF (error flags: 0x%08x)", bms->status.error_flags);
            }
            else if (bms_chg_allowed(bms)) {
                bms_chg_switch(bms, true);
                bms->status.state = BMS_STATE_NORMAL;
                LOGI("DIS -> NORMAL (error flags: 0x%08x)", bms->status.error_flags);
            }
#ifndef CONFIG_BQ769X2 // bq769x2 has built-in ideal diode control
            else {
        ...
        case BMS_STATE_NORMAL:
            if (!bms_dis_allowed(bms)) {
                bms_dis_switch(bms, false);
                bms->status.state = BMS_STATE_CHG;
                LOGI("NORMAL -> CHG (error flags: 0x%08x)", bms->status.error_flags);
            }
            else if (!bms_chg_allowed(bms)) {
                bms_chg_switch(bms, false);
                bms->status.state = BMS_STATE_DIS;
                LOGI("NORMAL -> DIS (error flags: 0x%08x)", bms->status.error_flags);
            }
            break;
    }
}
