...
void bms_set_error_flag(Bms *bms, uint32_t flag, bool value)
{
    ...
    if ((bms->status.error_flags & (1UL << flag)) != ((uint32_t)value << flag))
    {
        ...
        LOGI("Error flag %u changed to: %d", flag, value);
    }
}
...
void bms_shutdown(Bms *bms)
{
    ...
    bq769x0_write_byte(&bms->hi2c, BQ769X0_SYS_CTRL1, 0x0);
    bq769x0_write_byte(&bms->hi2c, BQ769X0_SYS_CTRL1, 0x1);
    bq769x0_write_byte(&bms->hi2c, BQ769X0_SYS_CTRL1, 0x2);
}
...
int bms_chg_switch(Bms *bms, bool enable)
{
    ...
    sys_ctrl2.byte = bq769x0_read_byte(&bms->hi2c, BQ769X0_SYS_CTRL2);
    sys_ctrl2.CHG_ON = enable;
    bq769x0_write_byte(&bms->hi2c, BQ769X0_SYS_CTRL2, sys_ctrl2.byte);
...
static void bms_update_balancing(Bms *bms)
{
    ...
    if (idle_secs >= bms->conf.bal_idle_delay && bms->status.cell_voltage_max > bms->conf.bal_cell_voltage_min && (bms->status.cell_voltage_max - bms->status.cell_voltage_min) > bms->conf.bal_cell_voltage_diff)
    {
        ...
        for (int section = 0; section < num_sections; section++)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if ((bms->status.cell_voltages[section * 5 + i] - bms->status.cell_voltage_min) > bms->conf.bal_cell_voltage_diff)
                {
                    int j = cell_counter;
                    while (j > 0 && bms->status.cell_voltages[section * 5 + cell_list[j - 1]] < bms->status.cell_voltages[section * 5 + i])
                    {
            ...
            for (int i = 0; i < cell_counter; i++)
            {
                ...
                bool adjacent_cell_collision = ((balancing_flags_target << 1) & balancing_flags) || ((balancing_flags << 1) & balancing_flags_target);
            ...
            LOGI("Setting CELLBAL%d register to: %s", section + 1, byte2bitstr(balancing_flags));

            bms->status.balancing_status |= balancing_flags << section * 5;

            // set balancing register for this section
            bq769x0_write_byte(&bms->hi2c, BQ769X0_CELLBAL1 + section, balancing_flags);
    ...
    else if (bms->status.balancing_status > 0)
    {
        ...
        for (int section = 0; section < num_sections; section++)
        {
            LOGI("Clearing Register CELLBAL%d", section + 1);
            bq769x0_write_byte(&bms->hi2c, BQ769X0_CELLBAL1 + section, 0x0);
        }
...
static int bms_apply_dis_scp(Bms *bms)
{
    ...
    bq769x0_write_byte(&bms->hi2c, BQ769X0_PROTECT1, protect1.byte);
...
static int bms_apply_dis_ocp(Bms *bms)
{
    ...
    bq769x0_write_byte(&bms->hi2c, BQ769X0_PROTECT2, protect2.byte);
...
static int bms_apply_cell_uvp(Bms *bms)
{
    ...
    bq769x0_write_byte(&bms->hi2c, BQ769X0_PROTECT3, protect3.byte);
...
static int bms_apply_cell_ovp(Bms *bms)
{
    ...
    bq769x0_write_byte(&bms->hi2c, BQ769X0_OV_TRIP, ov_trip);
...
void bms_read_temperatures(Bms *bms)
{
    ...
    adc_raw = (bq769x0_read_byte(&bms->hi2c, BQ769X0_TS1_HI_BYTE) & 0x3F) << 8 | bq769x0_read_byte(&bms->hi2c, BQ769X0_TS1_LO_BYTE);
    ...
    if (BOARD_NUM_THERMISTORS_MAX >= 2)
    { // bq76930 or bq76940
        adc_raw = (bq769x0_read_byte(&bms->hi2c, BQ769X0_TS2_HI_BYTE) & 0x3F) << 8 | bq769x0_read_byte(&bms->hi2c, BQ769X0_TS2_LO_BYTE);
        vtsx = adc_raw * 0.382;                 // mV
        rts = 10000.0 * vtsx / (3300.0 - vtsx); // Ohm
        tmp =
    ...
    if (BOARD_NUM_THERMISTORS_MAX == 3)
    { // bq76940
        adc_raw = (bq769x0_read_byte(&bms->hi2c, BQ769X0_TS3_HI_BYTE) & 0x3F) << 8 | bq769x0_read_byte(&bms->hi2c, BQ769X0_TS3_LO_BYTE);
        vtsx = adc_raw * 0.382;                 // mV
        rts = 10000.0 * vtsx / (3300.0 - vtsx); // Ohm
        tmp =
            1.0 / (1.0 / (273.15 + 25) + 1.0 / bms->conf.thermistor_beta * log(rts / 10000.0)); // K
...
void bms_read_current(Bms *bms)
{
    ...
    sys_stat.byte = bq769x0_read_byte(&bms->hi2c, BQ769X0_SYS_STAT);

    // check if new current reading available
    if (sys_stat.CC_READY == 1)
    {
        int adc_raw = bq769x0_read_word(&bms->hi2c, BQ769X0_CC_HI_BYTE);
        if (adc_raw < 0)
        {
            LOGI("Error reading current measurement");
            return;
        }
        // LOGI("ADC raw: %d\r\n", adc_raw);
        ...
        bq769x0_write_byte(&bms->hi2c, BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_CC_READY); // clear CC ready flag
    }
}
...
void bms_read_voltages(Bms *bms)
{
    ...
    {
        adc_raw = bq769x0_read_word(&bms->hi2c, BQ769X0_VC1_HI_BYTE + i * 2) & 0x3FFF;
    ...
    adc_raw = bq769x0_read_word(&bms->hi2c, BQ769X0_BAT_HI_BYTE);
...
void bms_update_error_flags(Bms *bms)
{
    ...
    if (bms->status.bat_temp_max > bms->conf.chg_ot_limit || (bms->status.error_flags & (1UL << BMS_ERR_CHG_OVERTEMP)))
    {
        error_flags_temp |= 1U << BMS_ERR_CHG_OVERTEMP;
    }

    if (bms->status.bat_temp_min < bms->conf.chg_ut_limit || (bms->status.error_flags & (1UL << BMS_ERR_CHG_UNDERTEMP)))
    {
        error_flags_temp |= 1U << BMS_ERR_CHG_UNDERTEMP;
    }

    if (bms->status.bat_temp_max > bms->conf.dis_ot_limit || (bms->status.error_flags & (1UL << BMS_ERR_DIS_OVERTEMP)))
    {
        error_flags_temp |= 1U << BMS_ERR_DIS_OVERTEMP;
    }

    if (bms->status.bat_temp_min < bms->conf.dis_ut_limit || (bms->status.error_flags & (1UL << BMS_ERR_DIS_UNDERTEMP)))
    {
...
//                 {
//                     LOGI("Attempting to clear XR error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_DEVICE_XREADY);
...
//                 {
//                     LOGI("Attempting to clear Alert error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_OVRD_ALERT);
//                     bms_chg_switch(bms, true);
//                     bms_dis_switch(bms, true);
//                 }
//             }
...
//                 {
//                     LOGI("Attempting to clear UV error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_UV);
//                     bms_dis_switch(bms, true);
//                 }
//             }
//             if (sys_stat.OV)
...
//                 {
//                     LOGI("Attempting to clear OV error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_OV);
//                     bms_chg_switch(bms, true);
//                 }
//             }
//             if (sys_stat.SCD)
...
//                 {
//                     LOGI("Attempting to clear SCD error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_SCD);
//                     bms_dis_switch(bms, true);
//                 }
//             }
//             if (sys_stat.OCD)
...
//                 if (sec_since_error % 60 == 0)
//                 {
//                     LOGI("Attempting to clear OCD error");
//                     bq769x0_write_byte(&bms->hi2c,BQ769X0_SYS_STAT, BQ769X0_SYS_STAT_OCD);
//                     bms_dis_switch(bms, true);
//                 }
//             }
//             sec_since_error++;
...
void bms_print_register(Bms *bms, uint16_t addr)
{
    uint8_t reg = bq769x0_read_byte(&bms->hi2c, (uint8_t)addr);
    LOGI("0x%.2X: 0x%.2X = %s", addr, reg, byte2bitstr(reg));
}

void bms_print_registers(Bms *bms)
{
    LOGI("0x00 SYS_STAT:  %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_SYS_STAT)));
    LOGI("0x01 CELLBAL1:  %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_CELLBAL1)));
    LOGI("0x04 SYS_CTRL1: %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_SYS_CTRL1)));
    LOGI("0x05 SYS_CTRL2: %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_SYS_CTRL2)));
    LOGI("0x06 PROTECT1:  %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_PROTECT1)));
    LOGI("0x07 PROTECT2:  %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_PROTECT2)));
    LOGI("0x08 PROTECT3:  %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_PROTECT3)));
    LOGI("0x09 OV_TRIP:   %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_OV_TRIP)));
    LOGI("0x0A UV_TRIP:   %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_UV_TRIP)));
    LOGI("0x0B CC_CFG:    %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_CC_CFG)));
    LOGI("0x32 CC_HI:     %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_CC_HI_BYTE)));
    LOGI("0x33 CC_LO:     %s\r\n", byte2bitstr(bq769x0_read_byte(&bms->hi2c, BQ769X0_CC_LO_BYTE)));
    /*&bms->hi2c, 
    LOGI("0x50 BQ769X0_ADCGAIN1:  %s", byte2bitstr(bq769x0_read_byte(BQ769X0_ADCGAIN1)));
    LOGI("0x51 BQ769X0_ADCOFFSET: %s", byte2bitstr(bq769x0_read_byte(BQ769X0_ADCOFFSET)));
    LOGI("0x59 BQ769X0_ADCGAIN2:  %s", byte2bitstr(bq769x0_read_byte(BQ769X0_ADCGAIN2)));
    */
}
